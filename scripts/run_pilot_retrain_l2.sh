#!/usr/bin/env bash
# Pilot: retrain one L2 concept RePS vectors (train-only pick layer), sweep mult, eye compare
# Usage: bash scripts/run_pilot_retrain_l2.sh L2_5
set -euo pipefail
PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$PROJECT_ROOT/easyedit_reps"
source env.sh

CONCEPT="${1:?usage: run_pilot_retrain_l2.sh L2_X}"
if [[ ! "$CONCEPT" =~ ^L2_[0-9]+$ ]]; then
  echo "Error: concept must be L2_* (got $CONCEPT)" >&2
  exit 1
fi

RUN="$PROJECT_ROOT/runs/pilot_retrain_${CONCEPT,,}"
VROOT="$EASYEDIT_REPS_ROOT/outputs/vectors/retrain_pilot"
SWEEP="$RUN/sweep"
...
# mult sweep: baseline ± one step (0.5), clamp [1.5, 5.0]
MULTS=$(python3 - <<PY
...
echo "=== Retrain pilot: $CONCEPT (baseline L${BASE_LAYER} m=${BASE_MULT}, sweep m=${MULTS}) ===" | tee "$LOG"
...
# default merge: baseline mult if present in sweep, else nearest
DEFAULT_M="$BASE_MULT"
...
# also emit one file per mult for official A/B
...
print(f"\nFiles: $SWEEP | default candidate: $OUT")
print("Official A/B: 绝地邮兵_result_pilot_retrain_${CONCEPT}_m*.json")
PY

echo "" | tee -a "$LOG"
echo "Done $CONCEPT. Review $LOG then pick mult for official submit." | tee -a "$LOG"

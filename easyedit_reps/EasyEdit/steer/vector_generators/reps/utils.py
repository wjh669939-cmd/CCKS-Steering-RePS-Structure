...
def _to_token_list(tokens) -> list[int]:
    """Normalize tokenizer outputs for transformers 4.x/5.x."""
...
def get_prefix_length(tokenizer, common_prefix=None):
    ...
    if common_prefix is None:
        ...
        tokens_a = _to_token_list(tokenizer.apply_chat_template(message_a, tokenize=True))
        tokens_b = _to_token_list(tokenizer.apply_chat_template(message_b, tokenize=True))
        prefix_length = 0

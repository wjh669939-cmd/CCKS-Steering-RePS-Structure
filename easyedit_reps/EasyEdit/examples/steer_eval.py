    ...
    for i, concept_train_data in train_datasets.items():
        ...
        if generate_response:
            ...
            if method == "prompt":
                ...
                elif method in ("reps", "reps_vector"):
                    hparam_key = "reps" if method == "reps" else method
                    vector_applier.hparams_dict[hparam_key].steer_vector_load_dir = f"{top_cfg.steer_vector_output_dirs[0]}/steer_eval_concept_{i}/reps_vector"
                else:
                    raise ValueError(f"Unknown method: {method}")
                
                if best_multipliers is not None:
                    hparam_key = "reps" if method == "reps" else method
                    vector_applier.hparams_dict[hparam_key].multipliers=[ int(best_multipliers[i]) ]
                hparam_key = "reps" if method == "reps" else method
                print(f"{color.BLUE}{vector_applier.hparams_dict[hparam_key]}{color.END}")
                vector_applier.apply_vectors()
            ...
            save_data = {
                ...
                'llm_description': concept_train_data[0].get('llm_description') or concept_train_data[0].get('concept_description'),

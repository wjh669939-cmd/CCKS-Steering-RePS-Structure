#!/usr/bin/env python3
"""Sweep RePS multipliers for L3 concepts only, merge with existing L1/L2 results."""
...
L3_CONCEPTS = [f"L3_{i}" for i in range(1, 9)]
VECTOR_ROOT = ROOT / "outputs/vectors/ccks_baseline_reps"
BASE_GENERATION = ROOT / "outputs/generation/ccks_baseline_reps/reps/all_generation_results_valid.json"
    ...
    return OmegaConf.create(
        {
            ...
            "generation_output_dir": str(ROOT / "outputs/generation/l3_sweep"),
...
def load_gold(path: Path) -> dict[tuple[str, str], dict]:
    ...
    return {(r["concept_id"], r["question"]): r for r in records if r["concept_id"].startswith("L3_")}
    ...
    for multiplier in multipliers:
        ...
        for concept_id in L3_CONCEPTS:
            ...
            for item in generated:
                ...
                row = score_item(gold, {"answer": answer})
                rows.append(row)
            hm = mean_hm(rows)
...
def main() -> None:
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=ROOT / "outputs/generation/l3_tuned",
    )
    ...
    (args.out_dir / "best_l3_multipliers.json").write_text(
        json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    (args.out_dir / "all_l3_sweep_runs.json").write_text(
        json.dumps(
            {
                cid: [
                    {"multiplier": r["multiplier"], "hm_proxy": r["hm_proxy"]} for r in runs
                ]
